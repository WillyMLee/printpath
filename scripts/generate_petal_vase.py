"""Generate PrintPath's single-piece, water-capable Petal Twist vase."""
...
def vertex(z: float, theta: float, inward: float = 0.0) -> Vec3:
    radius = outer_radius(z, theta) - inward
    return (radius * math.cos(theta), radius * math.sin(theta), z)


def top_vertex(theta: float, inward: float = 0.0) -> Vec3:
    # Eight shallow petals: the highest points remain at the approved 245 mm bound.
    z = HEIGHT - 2.5 + 2.5 * math.cos(FLUTES * theta)
    return vertex(HEIGHT, theta, inward)[:2] + (z,)
...
def generate() -> list[Triangle]:
    ...
    outer_rings = [[vertex(z, angle) for angle in angles] for z in regular_z]
    outer_rings.append([top_vertex(angle) for angle in angles])

    inner_z = [FLOOR] + [z for z in regular_z if z > FLOOR]
    inner_rings = [[vertex(z, angle, WALL) for angle in angles] for z in inner_z]
    inner_rings.append([top_vertex(angle, WALL) for angle in angles])
...
def write_binary_stl(path: Path, triangles: list[Triangle]) -> None:
    ...
    with path.open("wb") as handle:
        handle.write(b"PrintPath Petal Twist Vase".ljust(80, b"\0"))
...
if __name__ == "__main__":
    ...
    write_binary_stl(args.output, mesh)
